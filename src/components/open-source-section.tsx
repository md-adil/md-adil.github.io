    ...
    <section id={SectionId.OPEN_SOURCE} className="py-24 bg-slate-50">
      <div className="container mx-auto px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {PORTFOLIO_DATA.openSourceProjects?.map((project) => (
            <div
              key={project.id}
              className="bg-white p-6 rounded border border-slate-200 shadow-sm hover:shadow-md transition-all flex flex-col"
            >
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="px-3 py-1 bg-blue-50 text-blue-700 text-xs font-bold uppercase tracking-wide rounded-full">
                  {project.metric}
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {project.technologies.map((tech, i) => (
                  <span
                    key={i}
                    className="px-2 py-1 bg-slate-50 text-slate-500 text-xs font-medium border border-slate-100 rounded"
                  >
                    {tech}
                  </span>
                ))}
